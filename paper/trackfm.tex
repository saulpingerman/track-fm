...
\citep{ouyang2024tptrans} employed multi-head attention for spatiotemporal
features. Graph neural networks model vessel interactions.
% TODO(citations): restore GNN cites once author lists verified
% (k-GCN-LSTM, Physica A 2022; STMGF-Net, 2024) — old draft's keys had
% unverifiable/wrong attributions. Our work differs in evaluating
...
recovery objectives; the Pretrained Mobility Transformer \citep{wu2024pmt}
...
\citet{riveiro2018anomaly}; deep approaches include probabilistic sequence models
\citep{nguyen2018geotracknet} and multi-task frameworks \citep{park2025aisllm}.
% TODO(citations): old draft's chen2024wgananomalym could not be verified
% (likely miscited); replaced with GeoTrackNet.
Collision risk work increasingly integrates probabilistic trajectory
prediction \citep{jurkus2025collision}, which requires exactly the
...
saturates by 18M on containment (Figure~\ref{fig:scaling}); the projector
intervention is null at 2M and decisive at 18M, locating the bottleneck in
head-side mixing capacity rather than encoder width. Transfer shows the
same saturation on classification tasks but \emph{not} on regression
(Section~\ref{sec:transfer}). Learning-rate transfer illustrates the
recipe-over-scale theme sharply: a proxy-budget sweep placed the optimum
at $10^{-3}$ uniformly across widths, yet at the full budget $10^{-3}$
\emph{improves} the 2M model by 27\% (88 vs.\ 121) and \emph{degrades}
the 18M champion by 45\% (84 vs.\ 58) --- the optimum shifts with width,
and short-budget sweeps miss it in both directions. Mis-set LR at one
scale can masquerade as scale saturation, so we report the small-tier
projector null with that caveat.
\todo{xlarge-cone-projector scaling cell lands this week; full-data
flagship separately}

\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/scaling_containment.png}
    \caption{2h containment vs.\ encoder parameters at the compute-matched
    50M-sample budget. The fixed-geometry linear-head series saturates by
    18M; the cone linear-head series is flat from 18M to 116M; one
    projector layer at 18M (stars) is worth more than the full
    18M$\to$116M scale step.}
    \label{fig:scaling}
\end{figure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/dissociation_scatter.png}
    \caption{Forecasting containment vs.\ representation transfer across
    all encoders. The spectrum head (tied-best transfer, worst-of-modern
    containment) and the conditioned pretrain (the reverse) break any
    single-axis notion of model quality; open markers are fixed
    $\pm0.3^{\circ}$ canvases whose containment is computed on the 81\%
    of vessels they cover.}
