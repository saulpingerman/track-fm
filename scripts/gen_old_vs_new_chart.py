...
for i, (run, label, color) in enumerate(MODELS):
    ...
    for p, v in zip(pos, vals):
        ax.annotate(f"{v:.0f}", (p, v), ha="center", va="bottom",
                    fontsize=9.5, xytext=(0, 2), textcoords="offset points")

ax.set_yscale("log")
ax.set_ylim(3, 6000)
ax.set_xticks(x, [f"{b} forecast" for b in BUCKETS], fontsize=13)
ax.set_ylabel("grid cells searched to contain 90% of vessels\n"
              "(0.6 km² cells, ±0.3° population, log scale)", fontsize=13)
ax.set_title("Containment, project history vs current best "
             "(uniform metrics-v2 rescore, val)", fontsize=15, loc="left")
ax.legend(fontsize=11, loc="upper left", framealpha=0.9)
ax.grid(axis="y", which="both", alpha=0.25, zorder=0)
ax.spines[["top", "right"]].set_visible(False)
ax.set_facecolor("#fafafa")
fig.tight_layout()
out = "docs/figures/old_vs_new_containment.png"
