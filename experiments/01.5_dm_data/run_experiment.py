...
class Config:
    ...
    synthetic_data_path: str = "./data"  # Path for synthetic data

    # Data source
    use_synthetic: bool = False  # Use synthetic data instead of real AIS
    no_velocity_features: bool = False  # If True, only use [lat, lon, dt] (no sog/cog)
    use_raw_time: bool = False  # If True, use raw timestamp instead of dt
    ...
    # For real AIS (Danish EEZ): lat_mean=56.25, lon_mean=11.5
    # For synthetic data: lat_mean=30.0, lon_mean=-70.0
...
class AISTrackDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Build feature array
        if self.config.no_velocity_features:
            if self.config.use_raw_time:
                # [lat, lon, t] - raw cumulative time since start of sequence
                # Compute cumulative time from dt
                cumsum_t = np.cumsum(seq[:, 4])  # cumulative seconds
                t_norm = cumsum_t / 3600.0  # normalize by 1 hour
                features = np.zeros((len(seq), 3), dtype=np.float32)
                features[:, 0] = lat_norm
                features[:, 1] = lon_norm
                features[:, 2] = t_norm
            else:
                # [lat, lon, dt] - model must infer velocity from positions
                features = np.zeros((len(seq), 3), dtype=np.float32)
                features[:, 0] = lat_norm
                features[:, 1] = lon_norm
                features[:, 2] = dt_norm
        else:
            # Full features: [lat, lon, sog, cog_sin, cog_cos, dt]
...
def load_synthetic_data(config: Config, max_tracks: int = None) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray]]:
    """
    Load and preprocess synthetic data generated by dataset_gen.py.

    Computes derived features (dt, sog, cog) from raw lat/lon/time data.
    Returns data in same format as load_ais_data().
    """
    from pyproj.geod import Geod

    print("Loading synthetic data...", flush=True)
    data_path = Path(config.synthetic_data_path)
    ...
    if not train_path.exists() or not val_path.exists():
        raise FileNotFoundError(
            ...
            f"Run dataset_gen.py first to generate the data."
        )
    ...
    def process_tracks(df: pl.DataFrame, max_tracks: int = None) -> Dict[str, np.ndarray]:
        """Process raw synthetic data into feature arrays (vectorized)."""
        # Sort by track_id and time once
        df = df.sort(["track_id", "time"])

        # Get track lengths and filter
        track_lengths = df.group_by("track_id").agg(pl.len().alias("count"))
        valid_tracks = track_lengths.filter(pl.col("count") >= min_len)

        if max_tracks is not None and len(valid_tracks) > max_tracks:
            valid_tracks = valid_tracks.head(max_tracks)

        valid_ids = set(valid_tracks.select("track_id").to_series().to_list())
        df = df.filter(pl.col("track_id").is_in(valid_ids))

        # Extract arrays
        track_ids_arr = df.select("track_id").to_series().to_numpy()
        times_arr = df.select("time").to_series().to_numpy()
        lats_arr = df.select("lat").to_series().to_numpy().astype(np.float64)
        lons_arr = df.select("lon").to_series().to_numpy().astype(np.float64)

        n = len(lats_arr)

        # Vectorized dt computation
        dt_seconds = np.zeros(n, dtype=np.float32)
        dt_seconds[1:] = (times_arr[1:] - times_arr[:-1]).astype('timedelta64[ms]').astype(np.float64) / 1000.0

        # Vectorized geod.inv for all consecutive point pairs
        az_arr, _, dist_arr = geod.inv(lons_arr[:-1], lats_arr[:-1], lons_arr[1:], lats_arr[1:])

        # Compute sog and cog
        sog = np.zeros(n, dtype=np.float32)
        cog = np.zeros(n, dtype=np.float32)

        # Only compute where dt > 0 and same track
        same_track = track_ids_arr[:-1] == track_ids_arr[1:]
        valid_dt = (dt_seconds[1:] > 0) & same_track

        sog[1:] = np.where(valid_dt, dist_arr / dt_seconds[1:] * METERS_PER_SECOND_TO_KNOTS, 0.0)
        cog[1:] = np.where(same_track, az_arr % 360, 0.0)

        # Zero out dt at track boundaries
        dt_seconds[1:] = np.where(same_track, dt_seconds[1:], 0.0)

        # Build per-track feature arrays using numpy split (O(n) instead of O(n*m))
        track_data = {}

        # Find track boundaries efficiently
        _, unique_indices, unique_counts = np.unique(track_ids_arr, return_index=True, return_counts=True)

        # Sort by first occurrence to maintain order
        sort_order = np.argsort(unique_indices)
        unique_track_ids = track_ids_arr[unique_indices[sort_order]]
        counts = unique_counts[sort_order]

        # Split arrays at track boundaries
        split_indices = np.cumsum(counts)[:-1]

        lat_splits = np.split(lats_arr.astype(np.float32), split_indices)
        lon_splits = np.split(lons_arr.astype(np.float32), split_indices)
        sog_splits = np.split(sog, split_indices)
        cog_splits = np.split(cog, split_indices)
        dt_splits = np.split(dt_seconds, split_indices)

        for idx, track_id in enumerate(unique_track_ids):
            track_lats = lat_splits[idx]
            track_lons = lon_splits[idx]
            track_sog = sog_splits[idx].copy()  # Copy to allow modification
            track_cog = cog_splits[idx].copy()
            track_dt = dt_splits[idx].copy()

            # First point inherits from second
            if len(track_sog) > 1:
                track_sog[0] = track_sog[1]
                track_cog[0] = track_cog[1]
                track_dt[0] = track_dt[1]

            # Build feature array: [lat, lon, sog, cog, dt_seconds]
            features = np.column_stack([track_lats, track_lons, track_sog, track_cog, track_dt])
            track_data[track_id] = features
    ...
    train_data = process_tracks(train_df, max_tracks)
    print(f"    Loaded {len(train_data)} train tracks", flush=True)
    del train_df

    # Load and process validation data
    print("  Loading validation data...", flush=True)
    val_df = pl.read_parquet(val_path)
    val_data = process_tracks(val_df, max_tracks // 10 if max_tracks else None)
    print(f"    Loaded {len(val_data)} val tracks", flush=True)
    del val_df

    # Print summary
    total_train = sum(len(v) for v in train_data.values())
    total_val = sum(len(v) for v in val_data.values())
    print(f"  Total train positions: {total_train:,}", flush=True)
    print(f"  Total val positions: {total_val:,}", flush=True)
...
class CausalAISModel(nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        # 6 features with velocity (lat, lon, sog, cog_sin, cog_cos, dt)
        # 3 features without velocity (lat, lon, dt)
        num_input_features = 3 if config.no_velocity_features else 6
        self.input_proj = nn.Linear(num_input_features, config.d_model)
    ...
    def forward_train(self, features: torch.Tensor, horizon_indices: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Compute cumulative time for horizon conditioning
        if self.config.use_raw_time:
            # Raw time: features[:, :, 2] is cumulative time normalized by 3600
            cumsum_t = features[:, :, 2] * 3600.0  # Denormalize to seconds
            last_cumsum = cumsum_t[:, seq_len - 1]  # (batch,)
        else:
            # Delta time: need to cumsum
            # dt is at index 2 without velocity features, index 5 with velocity features
            dt_idx = 2 if self.config.no_velocity_features else 5
            dt = features[:, :, dt_idx] * self.config.dt_max  # Denormalize all dt
            cumsum_t = torch.cumsum(dt, dim=1)
            last_cumsum = cumsum_t[:, seq_len - 1]  # (batch,)
        ...
        for h in horizon_indices:
            ...
            cum_time = cumsum_t[:, tgt_idx] - last_cumsum
...
def compute_dead_reckoning_loss(features: torch.Tensor, targets: torch.Tensor,
    ...
    if config.use_raw_time:
        # Raw time: features[:, :, 2] is cumulative time normalized by 3600
        cumsum_t = features[:, :, 2] * 3600.0  # Denormalize to seconds
        # Compute dt from consecutive time values for velocity calculation
        dt_for_velocity = cumsum_t[:, seq_len - 1] - cumsum_t[:, seq_len - 2]
    else:
        # Delta time: dt is at index 2 without velocity features, index 5 with velocity features
        dt_idx = 2 if config.no_velocity_features else 5
        dt = features[:, :, dt_idx] * config.dt_max
        cumsum_t = torch.cumsum(dt, dim=1)
        dt_for_velocity = dt[:, seq_len - 1]

    # Compute velocity at last input position
    last_dlat = lat[:, seq_len - 1] - lat[:, seq_len - 2]
    last_dlon = lon[:, seq_len - 1] - lon[:, seq_len - 2]
    last_dt = dt_for_velocity.clamp(min=1.0)
    vlat = last_dlat / last_dt  # (batch,)
    vlon = last_dlon / last_dt

    # Cumulative time
    last_cumsum = cumsum_t[:, seq_len - 1]
    ...
    for h in horizon_indices:
        h = h.item()
        tgt_idx = seq_len + h - 1
        cum_time = cumsum_t[:, tgt_idx] - last_cumsum  # (batch,)
...
def main():
    ...
    parser.add_argument('--synthetic', action='store_true',
                        help='Use synthetic data from dataset_gen.py instead of real AIS data')
    # No velocity features flag
    parser.add_argument('--no-velocity', action='store_true',
                        help='Only use [lat, lon, dt] features (no sog/cog). Tests if model can infer velocity.')
    # Raw time flag (requires --no-velocity)
    parser.add_argument('--raw-time', action='store_true',
                        help='Use raw cumulative time t instead of delta time dt. Requires --no-velocity.')
    args = parser.parse_args()

    # Print header based on data source
    print("=" * 70, flush=True)
    if args.synthetic:
        print("EXPERIMENT 01.5: SYNTHETIC DATA VALIDATION", flush=True)
        print("Testing transformer + Fourier head on linear tracks", flush=True)
    else:
    ...
    # Apply synthetic data settings
    if args.synthetic:
        config.use_synthetic = True
        # Normalization for synthetic data region (200mi box around 30°N, -70°W)
        config.lat_mean = 30.0
        config.lon_mean = -70.0
        config.lat_std = 1.5  # ~100 mi north/south
        config.lon_std = 1.8  # ~100 mi east/west at this latitude
        # Synthetic tracks are shorter, adjust min_track_length
        config.min_track_length = 128 + config.max_horizon

    # Apply no-velocity setting
    if args.no_velocity:
        config.no_velocity_features = True
        print("Using position-only features: [lat, lon, dt]", flush=True)

    # Apply raw-time setting (requires no-velocity)
    if args.raw_time:
        if not args.no_velocity:
            print("ERROR: --raw-time requires --no-velocity", flush=True)
            import sys
            sys.exit(1)
        config.use_raw_time = True
        print("Using raw cumulative time t instead of delta time dt", flush=True)
        print("Features: [lat, lon, t]", flush=True)
    ...
    if config.use_synthetic:
        train_data, val_data = load_synthetic_data(config, max_tracks=args.max_tracks)
    else:
