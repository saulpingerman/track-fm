...
def visualize_single_prediction(model, features, config, sample_idx=0):
    ...
    with torch.no_grad():
        with autocast('cuda', enabled=config.use_amp):
            log_densities, targets = model.forward_train(sample)

    # Get the future predictions (last max_horizon)
    future_log_densities = log_densities[0, -max_horizon:, :, :].cpu()
    future_targets = targets[0, -max_horizon:, :].cpu()
    ...
    # Select which horizons to show (spread across max_horizon)
    if max_horizon <= 5:
        horizons_to_show = list(range(1, max_horizon + 1))
    else:
        # Show horizons 1, and then spread evenly up to max_horizon
        horizons_to_show = [1] + [int(i * max_horizon / 4) for i in range(1, 5)]
        horizons_to_show = sorted(set(horizons_to_show))[:5]

    # Plot for each selected horizon
    for h_idx, h in enumerate(horizons_to_show):
        if h_idx >= 5:
            break
        ax = axes[h_idx // 3, h_idx % 3]

        # Get predicted density - use h-1 to index into horizon-sized arrays
        density = torch.exp(future_log_densities[h-1]).numpy()
...
def visualize_track_with_context(model, val_dataset, config, sample_idx=0):
    ...
    # Use horizon 3 for visualization
    h = 3
    future_log_density = log_densities[0, -max_horizon + h - 1, :, :].cpu()
...
def visualize_multiple_tracks(model, val_dataset, config, num_tracks=6):
    ...
    for plot_idx, sample_idx in enumerate(indices):
        ...
        # Use horizon 3 (middle horizon) for visualization
        h = 3
        future_log_density = log_densities[0, -max_horizon + h - 1, :, :].cpu()
        density = torch.exp(future_log_density).numpy()
...
def visualize_max_horizon_grid(model, val_dataset, config, num_tracks=16):
    ...
    for plot_idx, sample_idx in enumerate(indices):
        ...
        # Get density for max horizon (last one)
        future_log_density = log_densities[0, -1, :, :].cpu()
        density = torch.exp(future_log_density).numpy()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Visualize AIS trajectory predictions')
    ...
    # Load config from experiment if available, otherwise use defaults
    ...
    # Use results subfolder if it exists, otherwise fall back to exp_dir
    ...
    # Visualization 1: Multiple tracks
    print("\nGenerating multi-track visualization...")
    fig1 = visualize_multiple_tracks(model, val_dataset, config, num_tracks=6)
    fig1.savefig(output_dir / 'viz_multiple_tracks.png', dpi=150, bbox_inches='tight')
    plt.close(fig1)
    print(f"  Saved: viz_multiple_tracks.png")

    # Visualization 1b: Track with full context (NEW - shows why tracks appear straight)
    print("\nGenerating track with context visualization...")
    for i, sample_idx in enumerate([100, 500, 1000]):
        fig1b = visualize_track_with_context(model, val_dataset, config, sample_idx=sample_idx)
        fig1b.savefig(output_dir / f'viz_track_context_{i+1}.png', dpi=150, bbox_inches='tight')
        plt.close(fig1b)
    print(f"  Saved: viz_track_context_1.png, viz_track_context_2.png, viz_track_context_3.png")

    # Visualization 2: Single track with all horizons
    print("\nGenerating single track detailed visualization...")
    # Get a batch of samples for the visualization
    sample_features = torch.stack([val_dataset[i] for i in range(100, 110)])
    fig2 = visualize_single_prediction(model, sample_features, config, sample_idx=0)
    fig2.savefig(output_dir / 'viz_single_track_horizons.png', dpi=150, bbox_inches='tight')
    plt.close(fig2)
    print(f"  Saved: viz_single_track_horizons.png")

    # Visualization 3: Horizon comparison
    print("\nGenerating horizon comparison visualization...")
    fig3 = visualize_horizon_comparison(model, val_dataset, config, sample_idx=50)
    fig3.savefig(output_dir / 'viz_horizon_comparison.png', dpi=150, bbox_inches='tight')
    plt.close(fig3)
    print(f"  Saved: viz_horizon_comparison.png")

    # Visualization 4: More tracks with different behaviors
    print("\nGenerating additional track visualizations...")
    np.random.seed(123)
    indices = np.random.choice(len(val_dataset), 9, replace=False)

    fig4, axes = plt.subplots(3, 3, figsize=(15, 15))
    axes = axes.flatten()
    ...
    for plot_idx, sample_idx in enumerate(indices):
        ...
        extent = [src_lon - grid_range, src_lon + grid_range,
                  src_lat - grid_range, src_lat + grid_range]
        ax.imshow(density, origin='lower', extent=extent, cmap='hot', alpha=0.8, aspect='auto')

        ax.plot(lon[:seq_len], lat[:seq_len], 'b-', linewidth=1.5, alpha=0.7)
        ax.plot(lon[seq_len-1], lat[seq_len-1], 'bo', markersize=6)
        ax.plot(actual_lon, actual_lat, 'g*', markersize=12, markeredgecolor='white')
        ax.plot(dr_lon, dr_lat, 'rx', markersize=10, markeredgewidth=2)
        ...
        exp_dlat = (density * xx).sum()
        exp_dlon = (density * yy).sum()
        ax.plot(src_lon + exp_dlon, src_lat + exp_dlat, 'c^', markersize=8)
        ...
        ax.set_xlim(src_lon - grid_range * 1.2, src_lon + grid_range * 1.2)
        ax.set_ylim(src_lat - grid_range * 1.2, src_lat + grid_range * 1.2)
        ax.set_title(f'Model: {model_error:.0f}m | DR: {dr_error:.0f}m', fontsize=10)
        ax.tick_params(labelsize=8)

    plt.suptitle('Model Predictions on 9 Validation Tracks (Horizon 3)\n'
                 'Blue=History, Green*=Actual, RedX=DeadReckon, Cyan^=Model',
                 fontsize=12, y=1.02)
    plt.tight_layout()
    fig4.savefig(output_dir / 'viz_9_tracks.png', dpi=150, bbox_inches='tight')
    plt.close(fig4)
    print(f"  Saved: viz_9_tracks.png")

    # Visualization 5: Maximum horizon (horizon 20) for 16 tracks
    print("\nGenerating maximum horizon (h=20) visualization for 16 tracks...")
    fig5 = visualize_max_horizon_grid(model, val_dataset, config, num_tracks=16)
    fig5.savefig(output_dir / 'viz_max_horizon_16tracks.png', dpi=150, bbox_inches='tight')
    plt.close(fig5)
    print(f"  Saved: viz_max_horizon_16tracks.png")

    print("\n" + "=" * 70)
    print("VISUALIZATION COMPLETE")
    print("=" * 70)
    print(f"\nOutput files in: {output_dir}")
    print("  - viz_multiple_tracks.png")
    print("  - viz_single_track_horizons.png")
    print("  - viz_horizon_comparison.png")
    print("  - viz_9_tracks.png")
    print("  - viz_max_horizon_16tracks.png")
