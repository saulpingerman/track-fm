...
def generate_horizon_video(model, val_dataset, config, sample_idx, output_path, max_horizon_video=40):
    ...
    # Future log densities (last max_horizon predictions)
    ...
    all_future_lat = lat[seq_len:seq_len + max_horizon]
    all_future_lon = lon[seq_len:seq_len + max_horizon]

    # Extend bounds a bit
    lat_min = min(src_lat - grid_range * 1.5, all_future_lat.min() - 0.005)
    lat_max = max(src_lat + grid_range * 1.5, all_future_lat.max() + 0.005)
    lon_min = min(src_lon - grid_range * 1.5, all_future_lon.min() - 0.005)
    lon_max = max(src_lon + grid_range * 1.5, all_future_lon.max() + 0.005)

    # Generate frames
    frames = []
    num_frames = min(max_horizon_video, max_horizon)  # Can only go up to model's max
    ...
    # Save as GIF (5 fps = 200ms per frame)
...
def main():
    ...
    parser.add_argument('--max-horizon', type=int, default=40,
                        help='Maximum horizon to visualize (default: 40)')
